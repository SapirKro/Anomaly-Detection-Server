...
int sapir(){
    ...
    ///float r=expectedValue(x ,N);
   /// float expectedValue2=((expectedValue(x,N))  * (expectedValue(x,N)));
  //  float xPower2 [10] = {};
  //  for(int i=0;i<10;i++) { xPower2[i] = (x[i]*x[i]); }
  ///  float expectedValue2= expectedValue(xPower2,N);
 //  printf( "%6.4lf", expectedValue2);
    float r=pearson(x,y,N);
